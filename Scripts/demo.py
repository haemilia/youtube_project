...
def cleanse_tabular_video_info(raw_df:pd.DataFrame, drop_vlc=False)-> pd.DataFrame:
    ...
    def cleanse_vid_df(df:pd.DataFrame, cleansing:dict):
        ...
        cleansed_df["views"] = df["views"].astype(int).apply(cleansing["views"])
        likes = df["likes"].astype(int).apply(cleansing["likes"])
        comments = df["comments"].astype(int).apply(cleansing["comments"])
